<template>
  <a-card :bordered="false">
    <a-form ref="formRef" :model="formState" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
      ...
      <a-form-item label="衡量标准" name="desc">
        <a-textarea v-model:value="formState.desc" placeholder="请输入衡量标准" />
      </a-form-item>
      <a-form-item name="type">
        <template #label>
          <div>客户<span style="color: rgba(0,0,0,.45);">（选填）<InfoCircleOutlined /></span></div>
        </template>
        <a-input v-model:value="formState.name" placeholder="请描述你服务的客户，内部客户直接 @姓名／工号" />
      </a-form-item>
      <a-form-item name="type">
        <template #label>
          <div>邀评人<span style="color: rgba(0,0,0,.45);">（选填）<InfoCircleOutlined /></span></div>
        </template>
        <a-input v-model:value="formState.name" placeholder="请直接 @姓名／工号，最多可邀请 5 人" />
      </a-form-item>
      <a-form-item name="type">
        <template #label>
          <div>权重<span style="color: rgba(0,0,0,.45);">（选填）<InfoCircleOutlined /></span></div>
        </template>
        <a-input-number v-model:value="formState.name" placeholder="请输入" /> %
      </a-form-item>
      <a-form-item label="目标公开" name="desc">
