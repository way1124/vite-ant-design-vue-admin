  ...
  <page-header-wrapper content="">
    <a-card :bordered="false">
      ...
      <div class="title">退货商品</div>
			<a-table :columns="goodsColumns" :row-key="record => record.id" :data-source="loadGoodsData" :pagination="false" style="margin-bottom: 24px">
				<template #id="{ text }"><a>{{ text }}</a></template>
			</a-table>

      <div class="title">退货进度</div>
			<a-table :columns="scheduleColumns" :row-key="record => record.key" :data-source="loadScheduleData" :pagination="false" style="margin-bottom: 24px">
				<template #status="{ text }"><a-badge :status="text" :text="statusFilter(text)"/></template>
			</a-table>
    </a-card>
  </page-header-wrapper>
</template>
...
import { IColumns } from "@/views/List/SearchTable/data";

const goodsColumns: IColumns[] = [
	{
		title: '商品编号',
		dataIndex: 'id',
		key: 'id',
...
const scheduleColumns: IColumns[] = [
	{
		title: '时间',
		dataIndex: 'time',
		key: 'time'
  ...
  components: {
	...
	setup() {
      ...
      return statusMap[status]
		...
		onMounted(async () => {
			const data1: any = await loadGoodsData()
			basicState.loadGoodsData = data1.data

			const data2: any = await loadScheduleData()
			basicState.loadScheduleData = data2.data
		})
