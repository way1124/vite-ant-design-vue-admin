...
const getData = (img: any) => new Promise((reslove, reject) => {
	let obj = {
		arrayBuffer: null,
	...
	getImageData(img).then(data => {
		...
		obj.orientation = getOrientation(data) as any;
		reslove(obj)
	}).catch(error => {
		reject(error)
	})
...
function getImageData(img: any): Promise<any> {
	let data = null;
	return new Promise((reslove, reject) => {
		if (img.src) {
			if (/^data\:/i.test(img.src)) { // Data URI
				...
				var http: XMLHttpRequest | null = new XMLHttpRequest();
